...
internal class GameState(
    ...
    private fun canTetraminoDisplace(tetramino: Tetramino, displacement: Coordinate): Boolean {
        for (block in tetramino.blocks) {
            if (block.state === BasicBlockState.ON_TETRAMINO) {
                val shifted = Coordinate.add(block.coordinate, displacement)
    ...
    fun lineRemove() {
        ...
        do {
            ...
            for (row in rows - 1 downTo 0) {
                ...
                for (column in 0 until columns) {
                    val tetramino: Tetramino? = tetraminos[board[row][column].tetraId]
                    ...
                    for (block in tetramino.blocks) {
                        ...
                        if (block.coordinate.y === row && block.coordinate.x === column) {
                            ...
                            ctr++
                            val upperTetramino: Tetramino = tetramino.copy(ctr)
                            ...
                            ctr++
                            val lowerTetramino: Tetramino = tetramino.copy(ctr)
    ...
    private fun adjustTheMatrix() {
        for (row in rows - 1 downTo 0) {
            for (column in 0 until columns) {
                val T: Tetramino? = tetraminos[board[row][column].tetraId]
    ...
    private fun shiftTillBottom(tetramino: Tetramino) {
        ...
        do {
            ...
            for (block in tetramino.blocks) {
                if (block.state === BasicBlockState.ON_EMPTY) continue
                val newCoordinate = Coordinate.add(block.coordinate, Coordinate(1, 0))
    ...
    init {
        ...
        tetraminos = SparseArray<Tetramino>()
